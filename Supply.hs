...
instance Show a => Show (Stream  a) where
  show s = showBeginning (streamToList s) 20 where
    showBeginning :: Show b => [b] -> Integer -> String
    showBeginning (x : xs) 1 = (show x) ++ "..."
    showBeginning (x : xs) n = (show x) ++ "," ++ showBeginning xs (n - 1)
...
instance Applicative Stream where
  pure x = rep x
  --(f :> fs) <*> (x :> xs) = f x :> (fs <*> xs)
  fs <*> xs = zipStreamsWith (\f x -> f x) fs xs
...
pureSupply a = S { runSupply = (\xs -> (a, xs))}
...
mapSupply2 f (S ga) (S gb) = S gc where
  gc s = (first, second) where
    first = f (fst (ga s)) (fst (gb (snd (ga s))))
    second = snd (gb (snd (ga s)))
